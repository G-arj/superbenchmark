...
class TestResultSummary(unittest.TestCase):
    ...
    def test_result_summary(self):
        ...
        summary = rs1._generate_summary()
        assert (len(summary) == 3)

        # Test - _merge_summary
        expected_summary_merge = [
            ['KernelLaunch', 'kernel-launch/event_overhead', 'mean', 0.009719],
            ['KernelLaunch', 'kernel-launch/event_overhead', 'p90', 0.00595],
            ['KernelLaunch', 'kernel-launch/event_overhead', 'min', 0.0055],
            ['KernelLaunch', 'kernel-launch/event_overhead', 'max', 0.1],
            ['KernelLaunch', 'kernel-launch/wall_overhead', 'mean', 0.010415],
            ['KernelLaunch', 'kernel-launch/wall_overhead', 'p90', 0.010686],
            ['KernelLaunch', 'kernel-launch/wall_overhead', 'min', 0.01006],
            ['KernelLaunch', 'kernel-launch/wall_overhead', 'max', 0.01071],
            ['NCCL', 'nccl-bw/allreduce_8388608_busbw:0', 'mean', 89.51],
            ['RDMA', 'ib-loopback/IB_write_8388608_Avg_*:0', 'mean', 23925.84125]
        ]
    ...
    def test_result_summary_run(self):
        ...
        ResultSummary().run(self.test_raw_data, self.test_rule_file, str(self.parent_path), 'excel')
        ...
        # Test - output in md
        ResultSummary().run(self.test_raw_data, self.test_rule_file, str(self.parent_path), 'md')
        ...
        # Test - output in html
        ResultSummary().run(self.test_raw_data, self.test_rule_file, str(self.parent_path), 'html')
