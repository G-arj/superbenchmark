...
class DataDiagnosis(RuleBase):
    ...
    def _get_metrics_and_baseline(self, rule, benchmark_rules, baseline):
    ...
    def _parse_rules_and_baseline(self, rules, baseline):
        ...
        try:
            if not rules or not baseline:
                logger.error('DataDiagnosis: get rules or baseline failed')
                return False
            self._sb_rules = {}
            ...
            for rule in benchmark_rules:
                ...
                self._get_metrics_and_baseline(rule, benchmark_rules, baseline)
